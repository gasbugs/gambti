...
function createDetail({ code, title, summary, strengths = [], growth = [] }) {
  const letters = code.split("");
  const alias = title.split("—")[1]?.trim() ?? title;
  const letterList = letters.map((letter) => LETTER_LABELS[letter]).filter(Boolean).join(", ");
  const trimmedSummary = summary.trim();
  const summarySentence = trimmedSummary.endsWith(".") ? trimmedSummary : `${trimmedSummary}.`;
...
export const resultSummaries = {
  ...
  ESTJ: {
    ...
    detail: {
      headline:
        "ESTJ는 MBTI 성격 유형으로 <strong>'경영자', '관리자'</strong>를 의미하며, 외향성(E), 감각형(S), 사고형(T), 판단형(J)의 약자입니다. 이 유형은 현실적이고 실용적이며, 체계적인 계획과 원리원칙을 중시하며, 목표 지향적이고 책임감이 강한 지도자형입니다.",
...
for (const [code, profile] of Object.entries(resultSummaries)) {
  if (!profile.detail) {
    profile.detail = createDetail({
      code,
      title: profile.title,
